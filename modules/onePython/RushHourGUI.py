...
class RushHourGUI:
    def __init__(self):
        ...
        Button(bg="#469683", highlightbackground="#469683",padx=33, pady=15, text="Think loud: ", command=lambda : self.drawClosed(0)).grid(row=5, column=9)
        self.infoText = Label(text="", padx=10)
        self.infoText.grid(row=6, column=9)
    ...
    def drawClosed(self, index):

        colors = ["#db1a1a", "#db811a", "#d8c81a", "#56d819", "#19d87e", "#14ccb3", "#1388cc", "#1215c9", "#7414ce",
                  "#c611ba", "#c60f6b", "#47557c", "#394f3f"]
        if(index < len(self.astar.CLOSED)):
            board = [["-" for c in range(6)] for r in range(6)]
            state = self.astar.CLOSED[index].getState()
            for i in range(len(state)):
                car = state[i]
                orientation = car[0]
                pieceSize = car[3]
            ...
            self.gui.update()
            sleep(0.2)
            self.drawClosed(index + 1)



    def findSolution(self, algo):
        self.infoText.configure(text="")
        self.astar = ASTAR(problem=RushHourBFS(6), goal=(5,2), initStateFile=self.file.get(), algo=algo)
