...
def runModule(epochs=50,nbits=4,lrate=0.03,showint=100,mbs=None,vfrac=0.1,tfrac=0.1,vint=100,sm=False):
    ...
    cman = Caseman(cfunc=case_generator,vfrac=vfrac,tfrac=tfrac, cfrac=0.7)

    # dims = [features, hidden layer,...,hidden layer n-1, hiden layer n, labels]
    dims = [11, 50,20, numberOfClasses]

    ann = Gann(dims=dims,cman=cman,lrate=lrate,showint=showint,mbs=mbs,vint=vint,softmax=sm, hiddenLayerActivationFunction="sigmoid", outputActivationFunction="relu", bounds=[-3, 3])
