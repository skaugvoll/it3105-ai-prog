...
def get_case_generator(data_name, nbits=4, numberOfClasses=None):
    ...
    if data_name == 'parity': return "(lambda: TFT.gen_all_parity_cases())"
    if data_name == 'dense': return "(lambda: TFT.gen_dense_autoencoder_cases())"
    if data_name == 'bit': return "(lambda: TFT.gen_vector_count_cases())"
    if data_name == 'segment': return "(lambda: TFT.gen_segment_vector_cases())"
